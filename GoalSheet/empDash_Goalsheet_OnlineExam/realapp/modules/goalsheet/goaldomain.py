...
#    print(em + ":" + managerEmail + ":" + str(tempId))
    emp_id = getEmpIdByEmail(em) 
    if not emp_id:
        print("AssignTemplate: Employee not found: " + str(em))
...
#        print('MSD-ID=%d, %s' % (mgs.id, mgs.title)) 
    ...
    for mgs in mgoalSectionList :
        ...
        if gsID  in goalSectionMap.keys() : # This was already and handled
            continue
        newgs = createGoalSectionFromMaster(mgs, emp_id, getAssessmentYearStart(year),getAssessmentYearEnd(year),  gs.id)
    ...
    for mg in mgoalsList :
        ...
        createGoalFromMaster(mg, ngs_id, emp_id, getAssessmentYearStart(year),getAssessmentYearEnd(year), gs.id, tempId  )
...
def getGoalSheetsForDc(dcEmail, year = '2018-2019') :
    ...
    finalGsList = []
    finalGsList = getGoalSheets(dcEmail, filterActiveEmps=False, year=year)
...
#        print("getting goal sheets for:"+ drEmail)
        finalGsList += getGoalSheets(drEmail.lower(), filterActiveEmps=False, year=year)
...
def requestGoalSheetApproval(sheet) :
    sheet.status = "Pending1stApproval" 
    ...
    return ("Goal Sheet is Pending1stApproval.")
...
def approveGoalSheet(comments, sheet, mgmtLevel) :
    ...
    if mgmtLevel == 2:
        ...
        sheet.status = "Pending2ndApproval"
    else :
#        print("mgmtLevel=" + str(mgmtLevel))
        sheet.status = "Approved"
    ...
    return ("Sheet Status changed to: " + sheet.status)
...
@cache.memoize(timeout=1800)
def getMgmtRelationship(sheet, loggedInEmail, loggedInEmpId=0 ) :
    ...
    if not sheet:
        return -1
    sheetOwnerEmpId = sheet.empId
    sheetAssessingMgr = sheet.assessingManager

    if not loggedInEmpId  :
        print("getMgmtRelationship called with insufficient information:" + loggedInEmail + ":" + str(loggedInEmpId) + ":" + str(sheetOwnerEmpId) )
        return -1
...
#    print("Getting LineManager for:" + str(sheetOwnerEmpId))
    ...
    #If its country-manager, just let go, don't bother
    if is_countryManager(loggedInEmail) : return 3

    if sheetAssessingMgr == loggedInEmpId : return 1 # Assessing manager is stated, don't think
...
#        print("Hit 1nd Level=" + str(mgrId) )
        if is_countryManager(loggedInEmail) : return 3 # Sridhar/Sriram/HR?
